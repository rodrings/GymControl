...
def search_position(lista, condicion):
    """
    Objetivo: encontrar la posición del primer elemento de 'lista' que cumpla 'condicion'.
    Parámetros: lista, la lista donde buscar; condicion, función que recibe una fila y devuelve True/False.
    Salida: posición del elemento o -1 si no existe.
    """
    posiciones = list(filter(lambda i: condicion(lista[i]), range(len(lista))))
...
def search_affiliate_position(code):
    ...
    """
    return search_position(affiliates, lambda fila: fila[AFF_CODE] == code)
...
def search_class_position(code):
    ...
    """
    return search_position(classes, lambda fila: fila[CLASS_CODE] == code)
